/**
 * Seeds a demo company and an active EvaluationTemplate for it.
 *
 * Run from the apps/api directory (bun auto-loads .env there):
 *   bun run scripts/seed-template.ts
 *
...
/**
 * The JSON schema the LLM output must conform to. Interpolated into the prompt
 * via {{schema}} and used by AnalysisService.validateOutput (top-level
 * `required` keys must be present; `score` must be a number 0–100).
 */
const outputSchema = {
  type: 'object',
  required: [
  ...
  properties: {
    ...
    score: {
      ...
      maximum: 100,
      description:
        'Overall call-quality score weighing execution against red flags and compliance issues.',
    },
    whatWasDone: {
      type: 'array',
      description:
        'Factual, chronological account of the stages/actions the rep covered. No judgement.',
      items: {
        type: 'object',
        ...
        properties: {
          stage: {
            type: 'string',
            description:
              'e.g. rapport, discovery, demo, objection_handling, pricing, next_steps',
          },
    ...
    redFlags: {
      type: 'array',
      description:
        'Risks to the deal or relationship: disengaged customer, unaddressed dealbreaker objections, over-discounting, talking over the customer, inaccurate product claims, unhandled competitor threats.',
      items: {
        type: 'object',
    ...
    objections: {
      type: 'array',
      description:
        'Every objection the prospect raised, with how the rep handled it.',
      items: {
        type: 'object',
        ...
        properties: {
          type: {
            type: 'string',
            description:
              'Short snake_case label for the objection, e.g. pricing, brand_recognition, timing, competitor, no_budget, needs_approval',
          },
          repHandling: {
            type: 'string',
          ...
          transcriptExcerpt: {
            type: 'string',
            description:
              'Verbatim 2-3 lines from the transcript covering the objection and response.',
          },
        },
      },
    },
    complianceIssues: {
      type: 'array',
      description:
        'Statements or behaviors that may violate regulation or company policy.',
      items: {
        type: 'object',
...
/**
 * The factual fields the rep form contains. The form extractor fills these from
 * the uploaded form image; the comparator reconciles them against the transcript.
 * Top-level `required` keys must be present after extraction.
 */
const formSchema = {
  type: 'object',
  required: [
...
async function main(): Promise<void> {
  ...
  // Lightweight schemas matching the app's collection names. strict:false so
  // we don't have to redeclare every field; timestamps and string UUID _ids
  // to match the app.
  ...
  // Respect the "one active template per company" partial unique index.
